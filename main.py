...
nu = (nu_real + 1j*damping(nu_real))*(diml/dimc) 

nu0 = 3e-3 *diml/dimc
sigma = 1e-3 *diml/dimc
dnu = derivfd(nu_real)
fnu = (-1)*exp( -(nu_real-nu0)**2/(2*sigma**2) )
fdnu=nu_real*dnu*fnu*(diml/dimc)**2.
...
for omegai in omegalist:
	...
	dknl_i_Grr,dknl_i_Ghr,dknl_i_adjGrr,dknl_i_adjGhr=([] for i in range(4))
	dknl_j_Grr,dknl_j_Ghr,dknl_j_adjGrr,dknl_j_adjGhr=([] for i in range(4))
	#~ dknl_k_Gphir,dknl_k_Chirr,dknl_k_Chihr=([] for i in range(3))
	#~ dknl_l_Grr,dknl_l_Ghr,dknl_l_Chirr,dknl_l_Chihr=([] for i in range(4))
	ssknl_Grr,ssknl_Ghr,ssknl_adjGrr,ssknl_adjGhr=([] for i in range(4))
	...
	for ell in xrange(ellmax+1):
		...
		#G-> G(x,x_source)
		#Chi->G(x,x_receiver)
		rrhoOmega2=(r_reduced*rho0)*(nu_real[omegai]*2*pi*diml/dimc)**2.
		Grr=xis *(fdnu[omegai]*4*pi**2.)
		Ghr=ps/rrhoOmega2 *(fdnu[omegai]*4*pi**2.)
		adjGrr=xir
		adjGhr=pr/rrhoOmega2
		
		div_Grr=derivfd(r_reduced**2.*Grr )/(r_reduced**2*scaling_reduced )
		div_Ghr=Ghr/r_reduced
		div_adjGrr=derivfd(r_reduced**2.*adjGrr)/(r_reduced**2*scaling_reduced )  
		div_adjGhr=adjGhr/r_reduced
                
                ssknl_Grr.append(div_Grr*rho0*c2_reduced)
		ssknl_Ghr.append(div_Ghr*rho0*c2_reduced)
		ssknl_adjGrr.append(div_adjGrr)
		ssknl_adjGhr.append(div_adjGhr)
		
		dknl_i_Grr.append((nu_real[omegai]*2*pi*diml/dimc)**2. *Grr)
		dknl_i_Ghr.append((nu_real[omegai]*2*pi*diml/dimc)**2. *Ghr)
		dknl_i_adjGrr.append(adjGrr)
		dknl_i_adjGhr.append(adjGhr)
		
		dknl_j_Grr.append(div_Grr*c2_reduced)
		dknl_j_Ghr.append(div_Ghr*c2_reduced)
		dknl_j_adjGrr.append(div_adjGrr)
		dknl_j_adjGhr.append(div_adjGhr)
	...
	np.savez(filename, r=r_reduced, xisrc=xisrc, psrc=psrc,xircv=xircv,prcv=prcv,
					...
					dknl_i_Grr=dknl_i_Grr,dknl_i_Ghr=dknl_i_Ghr,dknl_i_adjGrr=dknl_i_adjGrr,dknl_i_adjGhr=dknl_i_adjGhr,
					dknl_j_Grr=dknl_j_Grr,dknl_j_Ghr=dknl_j_Ghr,dknl_j_adjGrr=dknl_j_adjGrr,dknl_j_adjGhr=dknl_j_adjGhr,
                                        ssknl_Grr=ssknl_Grr,ssknl_Ghr=ssknl_Ghr,ssknl_adjGrr=ssknl_adjGrr,ssknl_adjGhr=ssknl_adjGhr)
